...
var findByIdAndRemove = function (id, callback) {
	roomModel.findByIdAndRemove(id, callback);
}
	...
	find(function(err, rooms){
		...
		rooms.every(function(room){
			...
			if(!pass) {
				...
				if(room.gameState != 1) { //then leaving the game has no consequence
					for(var i=0; i<4; i++) {
						if(room.usersSeated[i]==null || room.usersSeated[i].userId=="computer") continue;
						if(socket.id==room.usersSeated[i].socketId) {
							room.usersSeated[i]=null;
